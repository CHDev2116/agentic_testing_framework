...
class QualityOrchestrator:
    ...
    def run_pipeline(self, image_metrics):
        ...
        logger.info("\n--- [Pipeline Start] Analyzing %s ---", image_id)

        # --- Stage 1: 快速過濾 ---
        logger.info("Step 1: Running Gemma-2b for basic check...")
        ...
        if not gemma_res or not gemma_res.get("pass"):
            ...
            logger.info("Rejected by Gemma: %s", reason)
            return {
                "id": image_id,
                "final_verdict": "FAIL",
                "stage": "Filter",
        ...
        logger.info("Passed Gemma Filter. Reason: %s", gemma_res.get("reason"))

        # 在兩個大模型切換間隙，讓 CPU 稍微冷卻 0.5 秒
        time.sleep(0.5)

        # --- Stage 2: 深度分析 ---
        logger.info("Step 2: Dispatching to Llama-3.1 for deep analysis...")
        ...
        if not llama_res or llama_res.get("verdict") == "Error":
            logger.warning("Llama Analysis stopped by Safety Guard.")
            return {"id": image_id, "error": "Llama analysis timeout or error"}

        # --- Stage 3: 彙整最終報告 ---
        logger.info("Final Verdict: %s", llama_res.get("verdict"))
    ...
    def _parse_json(self, text):
        ...
        try:
            ...
            logger.warning("Parsing error logic triggered. Raw snippet: %s...", text[:50])
            return None
...
if __name__ == "__main__":
    ...
    logger.info("%s", json.dumps(report, indent=4))
